...
- `model`: `sonnet` (fast explorer via the Agent tool)
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per model on `fable`, `opus`, and `sonnet` (Agent tool `model` option), all in a single message.
