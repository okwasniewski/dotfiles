...
3. Pick the runners. Default to a mixed panel via the Agent tool `model` option: one on `fable`, one on `opus`, one on `sonnet`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive; vary the prompt angle per runner so candidates diverge.
